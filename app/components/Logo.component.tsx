...
const Logo = ({ className = "" }: Props) => {
  useEffect(() => {
    const pupils = document.querySelectorAll<SVGCircleElement>(".pupil");
    const pupilArea1 = pupils[0].getBoundingClientRect();
    const pupilArea2 = pupils[1].getBoundingClientRect();
    const r = pupilArea1.width * 0.5;
    pupils[0].style.transformBox = `fill-box`;
    pupils[0].style.transformOrigin = `-${0.3 * r}px -${0.25 * r}px `;
    pupils[1].style.transformBox = `fill-box`;
    pupils[1].style.transformOrigin = `-${0.3 * r}px -${0.25 * r}px `;

    document.addEventListener("mousemove", (e) => {
      const angle1 =
        Math.atan2(e.clientY - pupilArea1.top, e.clientX - pupilArea1.right) *
          (180 / Math.PI) +
        360;
      const angle2 =
        Math.atan2(e.clientY - pupilArea2.top, e.clientX - pupilArea2.right) *
          (180 / Math.PI) +
        360;
      pupils[0].style.transform = `translate(80%, 70%) rotate(${
        angle1 - 40 + "deg"
      })`;
      pupils[1].style.transform = `translate(80%, 70%) rotate(${
        angle2 - 40 + "deg"
      })`;
    });
  }, []);
  return (
    ...
    >
      ...
      <path
        className="transition-opacity opacity-30 group-hover:opacity-100"
      ...
      />
      <circle
        className="pupil transition-opacity opacity-100 group-hover:opacity-0"
        cx="74.3"
        cy="96.5"
        r="15.7"
      />
